(() => {
  ...
  function renderCommerceStatus(){const host=$('#commerceStatusGrid');host.replaceChildren();commerce().modules.forEach((module)=>{const card=document.createElement('article');card.className=`commerce-module commerce-module-${module.status.toLowerCase()}`;const status=document.createElement('span');status.className='status-pill';status.textContent=module.statusLabel;const title=document.createElement('h3');title.textContent=module.name;const description=document.createElement('p');description.textContent=module.description;card.append(status,title,description);host.append(card)})}
  function matching(){
  ...
  function add(item){const existing=state.cart.find((line)=>line.item.id===item.id);if(existing){if(existing.quantity>=commerce().maxQuantity){toast('数量の上限は99点です');return}existing.quantity+=1}else{state.cart.push({item,quantity:1})}save();renderCart();toast('カートに追加しました')}
  ...
  function openReview(){if(!state.cart.length){toast('商品をカートに追加してから確認してください');return}$('#reviewSummary').innerHTML=state.cart.map((line)=>`<p><strong>${escapeHtml(line.item.product_name_ja||line.item.name)}</strong><br>${escapeHtml(line.item.product_code||line.item.sku||'')} · 数量 ${line.quantity} · ${linePriceText(line)}</p>`).join('')+`<p><strong>商品小計（参考） ${$('#cartTotal').textContent}</strong></p>`;$('#reviewModal').setAttribute('aria-hidden','false');closeCart();$('.close-review').focus()}
